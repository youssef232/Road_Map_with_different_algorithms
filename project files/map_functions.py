...
client = openrouteservice.Client(
    key='5b3ce3597851110001cf6248040dd0ed2f244306ad892cc05d2ad442')

# creating map
map = folium.Map(location=[30.466940, 30.934191], zoom_start=10)

# creating marks on map
nodes_list = []
def create_marks(path):
    for node in path:
        nodes_list.append(coordinates[node])
        folium.Marker(
            location=coordinates[node], popup=folium.Popup(node), tooltip=node).add_to(map)
        map.save('map.html')

# creating routes on map
def create_routes():
    for node in range(len(nodes_list) - 1):
        route = client.directions(coordinates=[
        [nodes_list[node][1], nodes_list[node][0]], [nodes_list[node + 1][1], nodes_list[node + 1][0]]],
        profile='driving-car', format='geojson')
        folium.GeoJson(route, name='route').add_to(map)
        map.save('map.html')

    # getting directory path of the map
    directory_path = os.getcwd()
    filename = 'map.html'
    map_path = os.path.join(directory_path, filename)

    # open map
    webbrowser.open_new(map_path)
